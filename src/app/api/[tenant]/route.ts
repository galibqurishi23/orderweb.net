...
export async function GET(
    ...
    try {
        ...
        // Exclude system/auth routes from tenant resolution - be very specific
        const excludedPaths = [
            'auth', 'admin', 'system', 'super-admin', 'health', 'setup', 
            'debug-session', 'simple-check', 'check-admin', 'admin-login'
        ];
        if (excludedPaths.includes(tenant)) {
            return NextResponse.json(
                { error: `Route not handled by tenant resolver: ${tenant}` },
                { status: 404 }
            );
        }
