...
export async function POST() {
    try {
        console.log('Creating exact template to match the provided design...');

        // Exact replica of the design shown in the image
        const exactDesignTemplate = `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    ...
    <style>
        * {
        ...
        /* HEADER SECTION - Matching the blue gradient */
        .header {
        ...
        /* GIFT CARD - Exact match to the design */
        .gift-card {
            background: linear-gradient(135deg, {{PRIMARY_COLOR}} 0%, {{SECONDARY_COLOR}} 100%);
            border-radius: 16px;
            padding: 35px 30px;
        ...
        /* PERSONAL MESSAGE - Light blue background with left border */
        .personal-message {
...
<body>
    <div class="email-container">
        ...
        await db.execute(`
            ...
            SET html_content = ?,
                subject = 'Your Digital Gift Card from {{RESTAURANT_NAME}}'
            WHERE template_name LIKE '%gift%' OR template_type = 'gift_card'
        `, [exactDesignTemplate]);

        console.log('Template updated to exact design specification');
        ...
        return NextResponse.json({
            success: true,
            message: 'Email template updated to match exact design',
            template: result[0]
        });
