 ...
 */
export async function getMealDeals(tenantSlugOrId: string): Promise<MealDeal[]> {
    validateTenantId(tenantSlugOrId);

    try {
        // Check if it's a slug or an ID - if it's a slug, convert to ID
        ...
        if (!tenantSlugOrId.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
            ...
            tenantId = await getTenantIdFromSlug(tenantSlugOrId);
        }
        ...
        return rows.flatMap((row: any) => {
            let categories: MealDealCategory[] = [];
            try {
                categories = JSON.parse(row.categories) as MealDealCategory[];
            } catch (e) {
                console.error(`Invalid JSON in meal_deals.categories for id=${row.id}:`, row.categories);
                // Skip this row
                return [];
            }
            return [{
        ...
        console.error('Error fetching meal deals:', error && error.stack ? error.stack : error);
        if (error instanceof SyntaxError && error.message.includes('JSON')) {
            console.error('Likely cause: Invalid JSON in meal_deals.categories column.');
        }
        throw new Error('Failed to fetch meal deals: ' + (error && error.message ? error.message : 'Unknown error'));
    }
}
...
export async function getMealDealById(tenantId: string, dealId: string): Promise<MealDeal | null> {
    ...
    try {
        ...
        console.error('Error fetching meal deal:', error && error.stack ? error.stack : error);
        if (error instanceof SyntaxError && error.message.includes('JSON')) {
            console.error('Likely cause: Invalid JSON in meal_deals.categories column.');
        }
        throw new Error('Failed to fetch meal deal: ' + (error && error.message ? error.message : 'Unknown error'));
    }
}
...
export async function createMealDeal(tenantId: string, dealData: CreateMealDealRequest): Promise<MealDeal> {
    ...
    try {
        ...
        console.error('Error creating meal deal:', error && error.stack ? error.stack : error);
        if (error.code === 'ER_NO_REFERENCED_ROW_2') {
            throw new Error('Invalid tenant ID')
        }
        throw new Error('Failed to create meal deal: ' + (error && error.message ? error.message : 'Unknown error'));
    }
}
...
export async function updateMealDeal(tenantId: string, dealData: UpdateMealDealRequest): Promise<MealDeal> {
    ...
    try {
        ...
        console.error('Error updating meal deal:', error && error.stack ? error.stack : error);
        throw new Error('Failed to update meal deal: ' + (error && error.message ? error.message : 'Unknown error'));
    }
}
...
export async function deleteMealDeal(tenantId: string, dealId: string): Promise<void> {
    ...
    try {
        ...
        console.error('Error deleting meal deal:', error && error.stack ? error.stack : error);
        throw new Error('Failed to delete meal deal: ' + (error && error.message ? error.message : 'Unknown error'));
    }
}
...
export async function getActiveMealDeals(tenantId: string): Promise<MealDeal[]> {
    ...
    try {
        ...
        console.error('Error fetching active meal deals:', error && error.stack ? error.stack : error);
        if (error instanceof SyntaxError && error.message.includes('JSON')) {
            console.error('Likely cause: Invalid JSON in meal_deals.categories column.');
        }
        throw new Error('Failed to fetch active meal deals: ' + (error && error.message ? error.message : 'Unknown error'));
    }
}
