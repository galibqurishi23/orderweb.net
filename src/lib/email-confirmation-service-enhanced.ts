...
`;
  ...
  private static generateSmartFooter(globalCustomization: any): string {
    if (!globalCustomization) return '';

    let footerContent = '';
    
    // Add main footer text
    if (globalCustomization.footer_text && globalCustomization.footer_text.trim()) {
      footerContent += `<div style="text-align: center; padding: 20px; background: ${globalCustomization.footer_color || '#764ba2'}; border-radius: 8px; color: white; margin-bottom: 20px;">
        <p style="margin: 0; font-size: 16px;">${globalCustomization.footer_text}</p>
      </div>`;
    }
    
    // Build business information section
    const businessInfo = [];
    if (globalCustomization.restaurant_phone) businessInfo.push(`<strong>Phone:</strong> ${globalCustomization.restaurant_phone}`);
    if (globalCustomization.restaurant_email) businessInfo.push(`<strong>Email:</strong> ${globalCustomization.restaurant_email}`);
    if (globalCustomization.restaurant_address) businessInfo.push(`<strong>Address:</strong> ${globalCustomization.restaurant_address}`);
    if (globalCustomization.vat_number) businessInfo.push(`<strong>VAT Number:</strong> ${globalCustomization.vat_number}`);
    if (globalCustomization.business_registration) businessInfo.push(`<strong>Business Registration:</strong> ${globalCustomization.business_registration}`);
